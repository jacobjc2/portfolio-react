        ...
        <div className="flex justify-between items-center w-full h-20 text-white bg-black px-4 fixed">
            ...
            <ul className="hidden md:flex">
                {links.map(({id, link, href}) => (
                    ...
                    text-gray-400 hover:scale-105 duration-200"><Link to={href}>{link}</Link></li>
                ))}
            </ul>
            ...
            {nav && (
                ...
                w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-400">
                    {links.map(({id, link, href}) => (
                        <li key={id} className="px-4 cursor-pointer capitalize text-4xl py-6">
                            <a clas href={href}></a>{link}
                        </li>
                    ))}
                </ul>
            )}
